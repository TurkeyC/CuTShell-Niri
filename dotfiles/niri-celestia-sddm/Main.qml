// niri-celestia-sddm — Main.qml
// Faithful port of Celestia-Shell Center.qml / LockSurface.qml
...
Rectangle {
    ...
    Loader {
        ...
        onLoaded: console.log("[sddm-celestia] Palette loaded. Background color:", item.background)
    }
    Loader {
        id: settingsLoader
    ...
    onWallpaperPathChanged: console.log("[sddm-celestia] Wallpaper path resolved to:", wallpaperPath)
    ...
    Image {
        ...
        visible: false
        onStatusChanged: {
            if (status === Image.Ready) console.log("[sddm-celestia] Wallpaper loaded successfully")
            else if (status === Image.Error) console.warn("[sddm-celestia] Failed to load wallpaper from:", source)
        }
    }
